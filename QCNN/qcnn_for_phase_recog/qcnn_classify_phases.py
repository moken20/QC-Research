...
def instanciate_original_model(num_qubits):
    ...
    # my_qcnn.add_layer(my_qcnn.Layers[legacy_fully_connected_layer.name], kwargs={})　変更箇所
    my_qcnn.add_layer(my_qcnn.Layers['legacy_fc_layer_n3'], kwargs={})   #変更箇所 layer_n_{n_qubits}
...
def run_qcnn(unique_name, packaged_datasets, my_qcnn, initial_params):
    ...
    while (abs(successive_loss) > 1e-5) and (iteration_num < itterations):
        ...
        if iteration_num == 1:
            ...
            if successive_loss < 0:
                learning_rate *= 1.05              #lossが小さくなったらleaning rateを5%あげる
            else:
                learning_rate /= 2                 #lossが大きくなったらlearning rateを50%下げる
    ...
    if not os.path.exists("results/" + unique_name):
...
def main():
    ...
    import time
    t0 = time.time()
    run_qcnn(f"n{num_qubits}_TEST_5_ITERS_Original/", packaged_datasets, original_model, original_model_initial_params)
    print(time.time() - t0)

    # t0 = time.time()
    # run_qcnn(f"n{num_qubits}_TEST_5_ITERS_New/", packaged_datasets, new_model, new_model_initial_params)
    # print(time.time() - t0)
